...
struct DiffHighlightConcurrentParseTests {
  ...
  private func syntheticSource() -> String {
    ...
    for index in 0..<2500 {
      lines.append("func compute\(index)(value: Int) -> Int {")
      ...
      if index % 8 == 0 {
        ...
        lines.append("    \"\"\"")
        lines.append("    diagnostic message number \(index) spanning")
        lines.append("    several lines of literal text content")
        lines.append("    \"\"\"")
        lines.append("  )")
      }
      lines.append("  return doubled + \(index)")
      lines.append("}")
    }
  ...
  @Test func concurrentFanOutDoesNotCorruptIntoAllString() async throws {
    ...
    while start < lineCount {
      windows.append(start..<min(lineCount, start + 300))
      start += 150
    }
    var tasks: [Task<Void, Never>] = []
    for window in windows {
      tasks.append(Task { @MainActor in _ = await concurrent.styleRuns(for: blob, visibleLines: window) })
    }
